...
export function writeClientReports(summary, outDir = DEFAULT_REPORT_DIR) {
  ...
  fs.writeFileSync(
    path.join(outDir, 'client-report.md'),
    renderSummaryMarkdown(summary),
    'utf8',
  );
  fs.writeFileSync(
    path.join(outDir, 'client-report.html'),
    renderSummaryHtml(summary, overallTone),
    'utf8',
  );
  fs.writeFileSync(
    path.join(outDir, 'client-report-full.md'),
    renderFullMarkdown(summary),
    'utf8',
  );
  fs.writeFileSync(
    path.join(outDir, 'client-report-full.html'),
    renderFullHtml(summary, overallTone),
    'utf8',
  );
...
function renderSummaryMarkdown(summary) {
  ...
  return [
    ...
    '',
    '_See the full report for page-by-page and check details._',
    '',
  ]
...
function renderSummaryHtml(summary, overallTone) {
  const pageCount = summary.pageCount || summary.pages?.length || 0;
...
function renderFullHtml(summary, overallTone) {
  const pageCount = summary.pageCount || summary.pages?.length || 0;
