...
export function normalizeUrl(href, base) {
  try {
    ...
    // Drop common tracking noise so the same page isn't listed many times.
    const drop = [
      'utm_source',
      'utm_medium',
      'utm_campaign',
...
export function isCrawlable(url, origin) {
  ...
  if (
    /\.(pdf|jpg|jpeg|png|gif|svg|webp|zip|rar|exe|dmg|mp4|mp3|css|js|woff2?|ico|xml|txt|md|json|csv|rss|atom|map)$/i.test(
      pathName,
    )
  ) {
    return false;
  ...
  // Skip template/param routes that are not real pages.
  if (/[:*]/.test(url.pathname) || /\{[^}]+\}/.test(url.pathname)) return false;
  return true;
}
